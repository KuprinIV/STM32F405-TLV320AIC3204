...
#endif /* USE_HAL_PCD_REGISTER_CALLBACKS */
  	  HAL_PCDEx_SetRxFiFo(&hpcd_USB_OTG_FS, 0x80); // 512 bytes of Rx fifo
  	  // sum of all txFIFO's must be less or equal 0x140 words
	  HAL_PCDEx_SetTxFiFo(&hpcd_USB_OTG_FS, 0, 0x10); // 64 bytes for EP0
	  HAL_PCDEx_SetTxFiFo (&hpcd_USB_OTG_FS, 1, 0x19); // 100 bytes for EP1 (USB AUDIO IN)
	  HAL_PCDEx_SetTxFiFo (&hpcd_USB_OTG_FS, 2, 0x10); // 64 bytes for EP2 (HID)
	  HAL_PCDEx_SetTxFiFo (&hpcd_USB_OTG_FS, 3, 0x32); // 200 bytes for EP3 (USB AUDIO OUT + SYNC)
  }
  return USBD_OK;
}

...
typedef enum bg_error
{
	...
	bg_err_hardware_ble_not_supported                                                     =bg_errspc_hardware+6,   //Hardware does not support Bluetooth Low Energy.
	...
	bg_err_server_already_in_use                                                          =bg_errspc_bg+146,       //A server for the specified type of connection (SPP, HID, or L2CAP with specific PSM) was already started. Only one SPP server per SDP entry is supported; only one HID server altogether is supported; only one L2CAP sever per PSM is supported.
	bg_err_no_such_endpoint                                                               =bg_errspc_bg+147,       //The specified endpoint does not exist.
	bg_err_invalid_endpoint                                                               =bg_errspc_bg+148,       //The endpoint exists, but it does not support the BGAPI command executed.
            
	bg_err_invalid_endpoint_state                                                         =bg_errspc_bg+149,       //The endpoint could not execute the command in its current state.
	...
	bg_err_smp_no_bonding                                                                 =bg_errspc_smp+11,       //The bonding does not exist.
	...
	bg_err_bt_coarse_clock_adjustment_rejected_but_will_try_to_adjust_using_clock_dragging=bg_errspc_bt+64,        //The Central, at this time, is unable to make a coarse adjustment to the piconet clock, using the supplied parameters. Instead the Central will attempt to move the clock using clock dragging.
	bg_err_bt_refused_psm_not_supported                                                   =bg_errspc_bt+65,        //The remote end did not have a service that would accept L2CAP connection requests to the specified PSM.
	bg_err_bt_refused_security_block                                                      =bg_errspc_bt+66,        //The L2CAP connection attempt was blocked by the remote end due to security reasons.
	bg_err_bt_refused_no_resources                                                        =bg_errspc_bt+67,        //The remote end rejected the L2CAP connection due to lack of resources.
	bg_err_bt_acl_disconnected                                                            =bg_errspc_bt+68,        //The ACL carrying the L2CAP link was disconnected.
	bg_err_bt_psm_already_in_use                                                          =bg_errspc_bt+69,        //An L2CAP connection with the specified PSM is already open on the same ACL. For each ACL, only one connection can use the same PSM.
